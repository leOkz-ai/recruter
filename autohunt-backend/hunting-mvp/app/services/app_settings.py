...
PARTNER_BENCH_SOURCE_URLS_KEY = "partner_bench_source_urls"
...
def get_json_list_setting(engine, key: str) -> list[str]:
    raw = get_setting(engine, key)
    if not raw:
        return []
    try:
...
def set_json_list_setting(engine, key: str, value: list[str]) -> None:
    cleaned = [str(item).strip() for item in value if str(item or "").strip()]
    set_setting(engine, key, json.dumps(cleaned, ensure_ascii=False))
