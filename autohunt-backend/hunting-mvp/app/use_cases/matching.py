...
_WORK_FORMAT_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("remote", re.compile(r"(?i)\b(remote|remotely|home\s*office|wfh|удален\w*)\b")),
    ("hybrid", re.compile(r"(?i)\b(hybrid|гибрид\w*)\b")),
    ("office", re.compile(r"(?i)\b(office|onsite|on-site|on site|офис\w*|на месте)\b")),
)
_URLISH_RE = re.compile(r"(?i)(https?://|docs\.google\.com/|t\.me/)")
_MATCH_REF_NAME_RE = re.compile(r"(?im)^(?:Имя|Name|Column 1)\s*:\s*(.+?)\s*$")
_MATCH_REF_ROW_RE = re.compile(r"(?im)^Row:\s*(\d+)\s*$")
_MATCH_REF_URL_RE = re.compile(r"(?im)^External source URL:\s*(.+?)\s*$")
_MATCH_REF_RESUME_RE = re.compile(r"(?im)^(?:Резюме|Resume|CV|Ссылки на резюме)\s*:\s*(.+?)\s*$")
_MATCH_REF_ROLE_RE = re.compile(r"(?im)^(?:Роль|Role)\s*:\s*(.+?)\s*$")
_MATCH_REF_LOCATION_RE = re.compile(r"(?im)^(?:Локация|Location|Город|City)\s*:\s*(.+?)\s*$")
...
def _extract_text_label(text_value: str, pattern: re.Pattern[str]) -> str:
    match = pattern.search(str(text_value or ""))
    if not match:
        return ""
    return str(match.group(1) or "").strip()


def _extract_match_reference_identity(item: dict[str, Any]) -> dict[str, str]:
    source_meta = item.get("specialist_source_meta") or {}
    ...
    raw_text = str(item.get("specialist_original_text") or "")
    original_source_url = _extract_text_label(raw_text, _MATCH_REF_URL_RE)
    resume_url = (
        _extract_text_label(raw_text, _MATCH_REF_RESUME_RE)
        or str(structured.get("resume_url") or row_map.get("resume_url") or "").strip()
    )
    name = (
        _extract_text_label(raw_text, _MATCH_REF_NAME_RE)
        or str(structured.get("name") or row_map.get("name") or "").strip()
    )
    role = (
        _extract_text_label(raw_text, _MATCH_REF_ROLE_RE)
        or str(structured.get("role") or row_map.get("role") or item.get("specialist_role") or "").strip()
    )
    location = (
        _extract_text_label(raw_text, _MATCH_REF_LOCATION_RE)
        or str(structured.get("location") or row_map.get("location") or item.get("specialist_location") or "").strip()
    )
    manager_name = str(source_meta.get("manager_name") or "").strip() or "-"
    source_kind = str(source_meta.get("source_kind") or "file").strip() or "file"
    return {
...
def _score_sheet_candidate(identity: dict[str, str], row_map: dict[str, Any], structured: dict[str, Any]) -> int:
    row_resume = str(structured.get("resume_url") or row_map.get("resume_url") or "").strip()
    row_name = str(structured.get("name") or row_map.get("name") or "").strip()
    row_role = str(structured.get("role") or row_map.get("role") or "").strip()
    row_location = str(structured.get("location") or row_map.get("location") or "").strip()

    score = 0
    if identity["resume_url"] and row_resume and _normalize_ref_text(identity["resume_url"]) == _normalize_ref_text(row_resume):
        score += 100
    if identity["name"] and row_name and _normalize_ref_text(identity["name"]) == _normalize_ref_text(row_name):
        score += 40
    if identity["role"] and row_role and _normalize_ref_text(identity["role"]) == _normalize_ref_text(row_role):
        score += 20
    if identity["location"] and row_location and _normalize_ref_text(identity["location"]) == _normalize_ref_text(row_location):
        score += 10
    return score


def _resolve_live_sheet_reference(
    item: dict[str, Any],
    *,
    source_fetcher: MCPSourceFetcherClient | None,
    cache: dict[str, Any],
) -> dict[str, Any]:
    identity = _extract_match_reference_identity(item)
    source_url = str(identity.get("source_url") or item.get("specialist_source_url") or "").strip()
    if not source_fetcher or "docs.google.com/spreadsheets" not in source_url:
        return item

    fetch_result = cache.get(source_url)
    if fetch_result is None:
        try:
            fetch_result = source_fetcher.fetch_url(source_url)
        except Exception:
            fetch_result = None
        cache[source_url] = fetch_result

    if not fetch_result or not getattr(fetch_result, "ok", False) or getattr(fetch_result, "source_type", "") != "google_sheet":
        return item

    best_match = None
    best_score = 0
    for candidate in getattr(fetch_result, "items", []) or []:
        metadata = getattr(candidate, "metadata", {}) or {}
        row_map = metadata.get("row_map") or {}
        structured = metadata.get("structured_fields") or {}
        if not isinstance(row_map, dict):
            row_map = {}
        if not isinstance(structured, dict):
            structured = {}
        score = _score_sheet_candidate(identity, row_map, structured)
        if score > best_score:
            best_score = score
            best_match = candidate

    if best_score < 60 or best_match is None:
        item["specialist_live_sheet_name"] = None
        item["specialist_live_row_index"] = None
        item["specialist_live_source_display"] = None
        item["specialist_live_name"] = None
        item["specialist_live_location"] = None
        return item
    ...
    if not isinstance(structured, dict):
        structured = {}
    sheet_name = str(metadata.get("sheet_name") or metadata.get("table_name") or "").strip() or None
    row_index = getattr(best_match, "row_index", None)
    live_name = str(structured.get("name") or row_map.get("name") or "").strip() or None
    live_location = str(structured.get("location") or row_map.get("location") or "").strip() or None
    source_display_parts = [
        f"Менеджер: {identity['manager_name']}",
        f"Ссылка на файл: {source_url}",
    ]
    if sheet_name:
        source_display_parts.append(f"Лист: {sheet_name}")
    if row_index is not None:
        source_display_parts.append(f"Строка: {row_index}")
    if live_name:
        source_display_parts.append(f"Имя: {live_name}")
    if live_location:
        source_display_parts.append(f"Локация: {live_location}")
    source_display = "; ".join(source_display_parts)
    item["specialist_live_sheet_name"] = sheet_name
    item["specialist_live_row_index"] = row_index
    item["specialist_live_source_display"] = source_display
    item["specialist_live_source_url"] = source_url
    item["specialist_live_name"] = live_name
    item["specialist_live_location"] = live_location
    return item
...
def _build_stack_profile(entity_or_stack: Any, *, entity_kind: str = "GENERIC") -> dict[str, list[str]]:
    ...
    if role_profile_hit:
        primary = [str(role_profile_hit.get("id") or "").strip()]
    elif primary_from_role:
        primary = _unique_keep_order(primary_from_role)
    elif stack_profile_hit:
        primary = [str(stack_profile_hit.get("id") or "").strip()]
    else:
        primary = []
    if entity_kind == "VACANCY":
        primary = _unique_keep_order(primary)[:1] or _unique_keep_order(primary_from_stack)[:1]
    else:
        primary = _unique_keep_order(primary + primary_from_stack)
    return {
        "primary": primary,
...
def _catalog_primary_label(profile: dict[str, list[str]]) -> Optional[str]:
    for value in profile.get("primary") or []:
        candidate = str(value or "").strip()
        if candidate.startswith("profile_"):
            return candidate
    return None


def _meaningful_stack_tokens(profile: dict[str, list[str]]) -> set[str]:
    tokens: set[str] = set()
    for value in profile.get("primary") or []:
        candidate = str(value or "").strip()
        if candidate and not candidate.startswith("profile_"):
            tokens.add(candidate)
    for bucket in ("secondary", "tooling"):
        for value in profile.get(bucket) or []:
            candidate = str(value or "").strip()
            if candidate:
                tokens.add(candidate)
    return tokens
...
def _stack_match_details(required_entity: Any, candidate_entity: Any, *, required_kind: str = "GENERIC", candidate_kind: str = "GENERIC") -> dict[str, Any]:
    ...
    required_catalog = _catalog_primary_label(required_profile)
    candidate_catalog = _catalog_primary_label(candidate_profile)
    required_meaningful = _meaningful_stack_tokens(required_profile)
    ...
    if required_catalog or candidate_catalog:
        ...
        return {
            ...
            "overlap": meaningful_overlap or overlap,
            "required_catalog": required_catalog,
    ...
    return {
        "passes": bool(required_primary and candidate_primary and overlap and (not has_explicit_stack or meaningful_overlap)),
        "required_profile": required_profile,
        "candidate_profile": candidate_profile,
        "overlap": meaningful_overlap or sorted(overlap),
...
def _extract_work_format_tokens(entity_or_hit: Any) -> set[str]:
    entity = _coerce_match_entity(entity_or_hit)
    values = (
        entity.get("location"),
        entity.get("description"),
        entity.get("original_text"),
        entity.get("source_display"),
    )
    tokens: set[str] = set()
    for value in values:
        ...
        if not text_value:
            continue
        for label, pattern in _WORK_FORMAT_PATTERNS:
            if pattern.search(text_value):
                tokens.add(label)
    return tokens


def _work_format_compatible(left_entity: Any, right_entity: Any) -> bool:
    left_formats = _extract_work_format_tokens(left_entity)
    right_formats = _extract_work_format_tokens(right_entity)
    if not left_formats or not right_formats:
        return True
    return bool(left_formats & right_formats)
...
def rank_specialist_hit(vacancy: Any, hit: dict[str, Any]) -> Optional[dict[str, Any]]:
    ...
    if not details["passes"]:
        return None
    if not _work_format_compatible(vacancy, hit):
        return None
    ...
    ranked_hit["score_components"] = {
        ...
        "catalog_profile": details["overlap"][0] if details["overlap"] else None,
    }
...
def rank_vacancy_hit(bench: Any, hit: dict[str, Any]) -> Optional[dict[str, Any]]:
    ...
    if not details["passes"]:
        return None
    if not _work_format_compatible(bench, hit):
        return None
    semantic_score = _semantic_similarity(hit)
    secondary_score = _secondary_overlap_score(details["required_profile"], details["candidate_profile"])
...
def upsert_matches(engine, vacancy_id: str, specialist_hits: list[dict[str, Any]]) -> None:
    delete_sql = text("DELETE FROM matches WHERE vacancy_id = CAST(:vid AS uuid)")
    sql = text(
        """
    ...
    with engine.begin() as connection:
        connection.execute(delete_sql, {"vid": vacancy_id})
...
def upsert_matches_reverse(engine, specialist_id: str, vacancy_hits: list[dict[str, Any]]) -> None:
    delete_sql = text("DELETE FROM matches WHERE specialist_id = CAST(:sid AS uuid)")
    sql = text(
        """
        INSERT INTO matches(vacancy_id, specialist_id, similarity_score, rank)
        VALUES (:vid, :sid, :score, :rank)
    ...
    with engine.begin() as connection:
        connection.execute(delete_sql, {"sid": specialist_id})
...
def _revalidate_recent_match(item: dict[str, Any]) -> Optional[dict[str, Any]]:
    vacancy_entity = {
        "role": item.get("vacancy_role"),
        "company": item.get("vacancy_company"),
        "stack": item.get("vacancy_stack"),
        "grade": item.get("vacancy_grade"),
        "rate_min": item.get("vacancy_rate_min"),
        "rate_max": item.get("vacancy_rate_max"),
        "currency": item.get("vacancy_currency"),
        "location": item.get("vacancy_location"),
        "description": item.get("vacancy_description"),
        "original_text": item.get("vacancy_original_text"),
    }
    specialist_hit = {
        "id": item.get("specialist_id"),
        "role": item.get("specialist_role"),
        "stack": item.get("specialist_stack"),
        "grade": item.get("specialist_grade"),
        "rate_min": item.get("specialist_rate_min"),
        "rate_max": item.get("specialist_rate_max"),
        "currency": item.get("specialist_currency"),
        "location": item.get("specialist_location"),
        "description": item.get("specialist_description"),
        "original_text": item.get("specialist_original_text"),
        "source_display": item.get("specialist_source_display"),
        "is_internal": item.get("is_internal"),
        "is_own_bench_source": item.get("is_own_bench_source"),
        "semantic_sim": item.get("similarity_score"),
        "sim": item.get("similarity_score"),
    }
    ranked = rank_specialist_hit(vacancy_entity, specialist_hit)
    if not ranked:
        return None
    validated = dict(item)
    validated["similarity_score"] = float(ranked.get("sim") or 0.0)
    return validated


def list_recent_matches(engine, limit: int = 20, *, source_fetcher: MCPSourceFetcherClient | None = None) -> list[dict[str, Any]]:
    raw_limit = max(limit * 10, limit)
    query = text(
        """
        SELECT
          m.vacancy_id,
          ...
          v.currency AS vacancy_currency,
          v.location AS vacancy_location,
          v.description AS vacancy_description,
          v.original_text AS vacancy_original_text,
          (
            SELECT COALESCE(src.external_url, src.message_url)
            FROM sources src
            WHERE src.entity_type='vacancy' AND src.entity_id=v.id
          ...
          s.currency AS specialist_currency,
          s.location AS specialist_location,
          s.description AS specialist_description,
          s.original_text AS specialist_original_text,
          (
            SELECT COALESCE(src.external_url, src.message_url)
            FROM sources src
          ...
          (
            ...
            ORDER BY src.created_at DESC
            LIMIT 1
          ) AS specialist_source_meta,
          (
            SELECT COALESCE(src.source_meta ->> 'source_display', src.message_url)
            FROM sources src
            WHERE src.entity_type='specialist' AND src.entity_id=s.id
            ORDER BY src.created_at DESC
            LIMIT 1
          ) AS specialist_source_display,
          s.is_internal,
          EXISTS(
            SELECT 1
    ...
    with engine.begin() as connection:
        rows = connection.execute(query, {"raw_limit": raw_limit}).mappings().all()
...
def list_active_specialists_for_matching(engine, *, own_bench_url: Optional[str] = None) -> list[dict[str, Any]]:
    ...
    with engine.begin() as connection:
        rows = connection.execute(sql, {"own_url": own_url}).mappings().all()
    return [dict(row) for row in rows]


def list_active_vacancies_for_matching(engine) -> list[dict[str, Any]]:
    sql = text(
        """
        SELECT
          v.id, v.role, v.stack, v.grade, v.rate_min, v.rate_max, v.currency, v.location, v.description, v.original_text,
...
def build_manual_query_entity(text_value: str, *, mode: str) -> tuple[dict[str, Any], dict[str, Any] | None]:
    profile = detect_primary_catalog_profile(text_value)
    role = str(profile.get("role") or "").strip() if profile else ""
    stack_terms = [str(term or "").strip() for term in (profile.get("matched_terms") or [])] if profile else []
    stack_terms = [term for term in dict.fromkeys(stack_terms) if term]
    entity_kind = "VACANCY" if mode == "vacancy" else "BENCH"
    manual_role = ""
    if stack_terms:
        manual_primary = next(
            (
                ...
                for primary in _primary_labels_for_text(term)
                if primary and not str(primary).startswith("profile_")
            ),
            "",
        )
        if manual_primary:
            manual_role = _MANUAL_PROFILE_LABELS.get(manual_primary, manual_primary)
    if not role:
        fallback_profile = _build_stack_profile({"role": text_value.strip(), "stack": [text_value]}, entity_kind="VACANCY" if mode == "vacancy" else "BENCH")
        fallback_primary = next((item for item in (fallback_profile.get("primary") or []) if item and not str(item).startswith("profile_")), "")
        if fallback_primary:
