...
def enqueue_partner_companies_sync(
    engine,
    *,
    source_url: str,
    ...
    return enqueue_job(
        ...
        payload={"source_url": source_url, "source_urls": list(source_urls or []), "reason": reason},
        dedupe_key=dedupe_key,
        max_attempts=3,
    )
...
def execute_job(engine, job: JobRecord, *, repo: Repo, ollama: OllamaClient, source_fetcher: MCPSourceFetcherClient | None = None) -> dict[str, Any]:
    ...
    if job.kind == "partner_companies_sync":
        ...
        extra_source_urls = [str(item or "").strip() for item in (payload.get("source_urls") or []) if str(item or "").strip()]
        counts = extract_partner_company_counts_from_sheet(source_url, source_fetcher)
        processed_sources = [source_url] if source_url else []
        upsert_partner_company_mentions(engine, counts, source_url=source_url)
        return {
            "source_url": source_url,
            "source_urls": processed_sources + [url for url in extra_source_urls if url and url != source_url],
            "sources_processed": len(processed_sources),
            "companies_synced": len(counts),
            "counts": counts,
        }
...
def schedule_interval_jobs(engine) -> list[JobRecord]:
    ...
    partner_source_urls = get_json_list_setting(engine, PARTNER_BENCH_SOURCE_URLS_KEY)
    ...
    if partner_source_url:
        ...
        queued.append(
            enqueue_partner_companies_sync(
                ...
                source_urls=partner_source_urls,
