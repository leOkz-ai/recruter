...
class ManualRunRequest(BaseModel):
    mode: Literal["vacancy", "bench"]
    text: str
    limit: int = Field(default=10, ge=1, le=50)


class ManualRunItem(BaseModel):
    id: str
    title: str
...
class ManualRunSection(BaseModel):
    id: str
    title: str
    empty_text: str
    items: list[ManualRunItem] = Field(default_factory=list)
...
class ManualRunResponse(BaseModel):
    ...
    items: list[ManualRunItem] = Field(default_factory=list)
    sections: list[ManualRunSection] = Field(default_factory=list)
...
class TelegramChannelItem(BaseModel):
    ...
    source_kind: Literal["chat", "channel", "vacancy", "bench"] = "chat"
    is_active: bool = True
...
class PartnerBenchSourceItem(BaseModel):
    name: str
    source_url: str
    registry_url: Optional[str] = None


class PartnerBenchSourceListResponse(BaseModel):
    items: list[PartnerBenchSourceItem]
