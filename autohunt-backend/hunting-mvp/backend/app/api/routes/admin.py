...
from app.services.app_settings import (
    ...
    PARTNER_BENCH_SOURCE_URLS_KEY,
    PARTNER_COMPANIES_SOURCE_URL_KEY,
    get_effective_setting,
    get_json_list_setting,
    get_setting,
)
from app.services.partner_companies import extract_partner_company_sources_from_sheet
...
from backend.app.api.schemas import (
    ...
    PartnerBenchSourceItem,
    PartnerBenchSourceListResponse,
    TelegramChannelItem,
...
def run_partner_sync() -> ImportJobAcceptedResponse:
    ...
    source_urls = get_json_list_setting(engine, PARTNER_BENCH_SOURCE_URLS_KEY)
    job = enqueue_partner_companies_sync(engine, source_url=source_url, source_urls=source_urls, reason="manual_api")
    write_audit_event(
        engine,
        event_type="partner_sync.enqueued",
        entity_type="job",
        entity_id=job.id,
        payload={"source_url": source_url, "source_urls_count": len(source_urls)},
    )
    return ImportJobAcceptedResponse(job_id=job.id, status=job.status)


@router.get("/partner-bench-sources", response_model=PartnerBenchSourceListResponse)
def list_partner_bench_sources() -> PartnerBenchSourceListResponse:
    engine = get_engine()
    source_fetcher = get_source_fetcher()
    registry_url = get_effective_setting(engine, PARTNER_COMPANIES_SOURCE_URL_KEY, "PARTNER_COMPANIES_SOURCE_URL", "")
    if not registry_url:
        return PartnerBenchSourceListResponse(items=[])
    items = [
        PartnerBenchSourceItem(**entry)
        for entry in extract_partner_company_sources_from_sheet(registry_url, source_fetcher)
    ]
    return PartnerBenchSourceListResponse(items=items)
