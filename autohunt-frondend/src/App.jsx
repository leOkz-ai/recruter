...
const roleProfiles = [
  { id: "python", terms: ["python", "питон", "питонист", "django", "fastapi", "flask"] },
  { id: "java", terms: ["java", "spring", "spring boot", "kotlin"] },
  { id: "php", terms: ["php", "bitrix", "laravel", "symfony"] },
  { id: "frontend", terms: ["frontend", "front-end", "react", "vue", "angular", "javascript", "typescript"] },
  { id: "designer", terms: ["designer", "design", "дизайнер", "ux", "ui", "figma", "product design"] },
  { id: "analyst", terms: ["analyst", "analysis", "аналитик", "системный аналитик", "business analyst", "business analysis"] },
  { id: "qa", terms: ["qa", "quality assurance", "тестировщик", "tester", "test automation", "авто qa", "manual qa"] },
  { id: "devops", terms: ["devops", "sre", "platform engineer", "kubernetes", "terraform", "argo cd", "argocd"] },
  { id: "data", terms: ["data engineer", "data analyst", "data scientist", "etl", "bi", "analyst data", "ml engineer"] },
  { id: "mobile", terms: ["android", "ios", "mobile", "swift", "kotlin developer", "flutter", "react native"] },
  { id: "product", terms: ["product manager", "project manager", "pm", "delivery manager", "scrum master"] }
];
...
function pickActualSpecialistLink(...values) {
...
function buildDialogLinkRow(label, href, text) {
  ...
  return {
    label,
    value: text ?? normalizedHref,
    href: normalizedHref
  };
}

function resolveCollectionItemSourceUrl(item) {
  const meta = item?.meta ?? item ?? {};
  return pickActualSpecialistLink(
    meta.source_url,
    meta.sourceUrl,
    meta.source_display,
    meta.original_text
...
function enrichManualRunResultWithLinks(result, mode, sourceCollections) {
  if (!result || typeof result !== "object") {
    return result;
  }

  const sourceItems = mode === "vacancy" ? sourceCollections.bench : sourceCollections.vacancies;
  const sourceUrlById = new Map(
    (sourceItems || []).map((item) => {
      const meta = item?.meta ?? item ?? {};
      return [String(meta.id ?? ""), resolveCollectionItemSourceUrl(item)];
    }).filter(([id, url]) => id && url)
  );

  const enrichItem = (item) => ({
    ...item,
    source_url: String(item?.source_url ?? "").trim() || sourceUrlById.get(String(item?.id ?? "")) || ""
  });

  return {
    ...result,
    items: Array.isArray(result.items) ? result.items.map(enrichItem) : [],
    sections: Array.isArray(result.sections)
      ? result.sections.map((section) => ({
          ...section,
          items: Array.isArray(section.items) ? section.items.map(enrichItem) : []
        }))
      : []
  };
}
...
function extractRoleProfileIds(value) {
  const haystack = ` ${normalizeText(value)} `;
  return roleProfiles
    .filter((profile) => profile.terms.some((term) => haystack.includes(` ${normalizeText(term)} `)))
    .map((profile) => profile.id);
}

function extractRoleLine(value) {
  ...
  return lines.find((line) =>
    /developer|engineer|designer|аналитик|дизайнер|разработчик|devops|qa|tester|тестировщик/i.test(line)
  ) ?? "";
}
...
function buildRunCorpus(mode, collections) {
  if (mode === "bench") {
    return collections.vacancies.map((item) => ({
      id: item[0],
      title: item.meta?.role ?? item.meta?.title ?? item[0],
      ...
      roleProfiles: extractRoleProfileIds([
        item.meta?.role,
        item.meta?.title,
  ...
  return collections.bench.map((item) => ({
    id: item[0],
    title: item.meta?.name ?? item.meta?.candidate ?? item.meta?.specialist ?? item[0],
    subtitle: `${item[2]} • ${item[4]}`,
    meta: item[3],
    tags: item[1],
    kindLabel: "Бенч",
...
function buildManualRunResult(queryText, mode, collections) {
  ...
  const explicitRoleProfiles = extractRoleProfileIds([queryText, extractRoleLine(queryText)].join(" "));
  if (!explicitRoleProfiles.length) {
    return {
      title: mode === "bench" ? "Топ-10 вакансий для специалиста" : "Топ-10 специалистов под вакансию",
      description: "Не удалось надёжно определить профиль из текста. Укажите явную роль: например, Python, дизайнер, QA, DevOps.",
      items: []
    };
  }
  ...
  const ranked = buildRunCorpus(mode, collections)
    .map((item) => {
      const hasRoleMatch = item.roleProfiles.some((profile) => explicitRoleProfiles.includes(profile));
      ...
      const score = hasRoleMatch && targetSet.size
        ? Math.round((overlap.length / querySet.size) * 55 + (overlap.length / targetSet.size) * 20 + 25)
        : 0;
  ...
  return {
    ...
    description: ranked.length
      ? "Показаны только записи с подтверждённым совпадением по профилю и ключевым словам."
...
function App() {
  ...
  const [matchesScope, setMatchesScope] = React.useState("own");
  ...
  const loadSettings = React.useCallback(async () => {
    try {
      ...
      const ownBenchSetting = items.find((item) => item.key === "own_bench_source_url");
      setOwnBenchSourceUrl(String(ownBenchSetting?.value ?? "").trim());
    } catch (error) {
      console.error("Failed to load settings:", error);
    }
  }, []);

  const loadPartnerBenchSources = React.useCallback(async () => {
    try {
      const payload = await api.getPartnerBenchSources();
      setPartnerBenchSources(Array.isArray(payload?.items) ? payload.items : []);
    } catch (error) {
      console.error("Failed to load partner bench sources:", error);
      setPartnerBenchSources([]);
    }
  }, []);
  ...
  React.useEffect(() => {
    ...
    loadSettings();
    loadPartnerBenchSources();
  }, [loadCollection, loadTelegramChannels, loadSettings, loadPartnerBenchSources]);
  ...
  const ownBenchCount = collections.bench.filter((item) => Boolean(item?.meta?.is_internal)).length;
  const partnerBenchCount = partnerBenchSources.length;
  const dashboardStatsView = [
    { ...dashboardStats[0], value: String(collections.inbox.length) },
    { ...dashboardStats[1], value: String(collections.vacancies.length) },
    { label: "Наш бенч", value: String(ownBenchCount), accent: "purple" },
    { label: "Бенч партнёров", value: String(partnerBenchCount), accent: "blue" },
    { ...dashboardStats[3], value: String(collections.matches.length) }
  ];
  ...
  const handleRunProcess = async () => {
    try {
      const result = await api.runManualMatch({
        mode: runMode,
        text: runText,
        limit: 10
      });
  ...
  const handleOwnBenchSync = async () => {
    const normalizedUrl = String(ownBenchSourceUrl ?? "").trim();
    if (!normalizedUrl) {
      setNotice("Вставьте ссылку на наш бенч перед обновлением.");
      return;
    }
    setIsOwnBenchSyncing(true);
    try {
      await api.updateSettings({
        items: [{ key: "own_bench_source_url", value: normalizedUrl }]
      });
      await api.runOwnBenchSync();
      setNotice("Обновление нашего бенча запущено.");
      window.setTimeout(() => {
        Promise.all([loadCollection("bench"), loadCollection("matches")]).catch((error) => {
          console.error("Failed to refresh own bench after sync:", error);
        });
      }, 1500);
    } catch (error) {
      console.error("Failed to sync own bench:", error);
      setNotice("Не удалось запустить обновление нашего бенча.");
    } finally {
      setIsOwnBenchSyncing(false);
    }
  };
  ...
  const handleRowAction = (kind, payload) => {
    ...
    const sourceUrl = extractFirstUrl(meta.source_url) || extractFirstUrl(meta.source_display);
    const cvUrl = pickActualSpecialistLink(meta.source_display, meta.original_text, meta.source_url);
    const inboxPostUrl = String(meta.message_url ?? meta.messageUrl ?? "").trim();
    ...
    const dialogByKind = {
      inbox: {
        ...
        rows: [
          ...
          ["Статус", payload[4]],
          ["Дата", payload[5]],
          buildDialogLinkRow("Ссылка на пост", inboxPostUrl, "Открыть пост")
        ]
      },
      vacancies: {
        ...
        rows: [
          ...
          ["Статус", payload[4]],
          ["Дата", payload[5]],
          buildDialogLinkRow("Источник вакансии", sourceUrl, "Открыть источник")
        ].filter(Boolean)
      },
      bench: {
        ...
        rows: [
          ...
          ["Статус", payload[5]],
          ["Дата", payload[6]],
          buildDialogLinkRow("CV", cvUrl, "Открыть CV")
        ].filter(Boolean)
      },
      logs: {
        title: `Лог: ${payload[2] ?? "—"}`,
      ...
      <main className="content">
        ...
        {current.id === "bench" && (
          <BenchPage
            ...
            ownBenchSourceUrl={ownBenchSourceUrl}
            onOwnBenchSourceUrlChange={setOwnBenchSourceUrl}
            onOwnBenchSync={handleOwnBenchSync}
            isOwnBenchSyncing={isOwnBenchSyncing}
        ...
        {current.id === "matches" && (
          <MatchesPage
            items={collections.matches}
            scope={matchesScope}
            onScopeChange={setMatchesScope}
        ...
        {current.id === "settings" && (
          <SettingsPage
            settings={settings}
            theme={settingsTheme}
            partnerBenchSources={partnerBenchSources}
              ...
              {matches.map((match) => (
                <div key={match.title} className="compact-list__item">
                  <div>
                    ...
                    {match.vacancy.sourceUrl || match.candidate.sourceUrl ? (
                      <div className="compact-list__text">
                        {match.vacancy.sourceUrl ? (
                          <a href={match.vacancy.sourceUrl} target="_blank" rel="noreferrer">
                            Открыть вакансию
                          </a>
                        ) : null}
                        {match.vacancy.sourceUrl && match.candidate.sourceUrl ? " · " : null}
                        {match.candidate.sourceUrl ? (
                          <a href={match.candidate.sourceUrl} target="_blank" rel="noreferrer">
                            Открыть специалиста
                          </a>
                        ) : null}
                      </div>
...
function BenchPage({
  ...
  return (
    <section className="page">
      ...
      {benchScope === "own" ? (
        <Panel title="Обновление нашего бенча" subtitle="Вставьте актуальную ссылку и запустите синхронизацию">
          <div className="form-grid">
            <label className="field">
              <span>Ссылка на наш бенч</span>
              <input
                value={ownBenchSourceUrl}
                onChange={(event) => onOwnBenchSourceUrlChange(event.target.value)}
                placeholder="https://docs.google.com/..."
              />
              <small>Будет сохранена в настройках и использована для обновления нашего бенча.</small>
            </label>
          </div>
          <div className="editor-footer">
            <button className="btn btn--primary" type="button" onClick={onOwnBenchSync} disabled={isOwnBenchSyncing}>
              {isOwnBenchSyncing ? "Обновляем..." : "Обновить наш бенч"}
            </button>
          </div>
        </Panel>
      ) : null}
...
function MatchesPage({ items, scope, onScopeChange, sortOrder, onSortChange, onRefresh, onAction }) {
  ...
  const visibleItems = [...(scope === "partner" ? partnerBenchMatches : ownBenchMatches)].sort((left, right) =>
    sortOrder === "desc" ? right.score - left.score : left.score - right.score
  );
  ...
  return (
    <section className="page">
      ...
      <div className="sort-switch" role="group" aria-label="Сортировка совпадений">
        <div className="panel-actions">
          <button
            className={`ghost-tab ${scope === "own" ? "is-active" : ""}`}
            type="button"
          ...
          <button
            className={`ghost-tab ${scope === "partner" ? "is-active" : ""}`}
            type="button"
            onClick={() => onScopeChange("partner")}
          >
            Бенч партнёров совпадений
          </button>
        </div>
        <div className="panel-actions sort-switch__sort">
          <button
      ...
      <div className="match-list">
        {visibleItems.length ? (
          visibleItems.map((match, index) => (
            <article key={`${match.source}-${match.vacancy.company}-${match.candidate.name}-${match.date}-${index}`} className="match-card">
              ...
              <div className="match-card__content">
                ...
                <CompareCard
                  ...
                  subtitle={match.candidate.role && match.candidate.role !== match.candidate.name ? match.candidate.role : ""}
                  stack={match.candidate.stack}
                  grade={match.candidate.grade}
                  rate={match.candidate.rate}
                  location={match.candidate.location}
                  identityHint={match.candidate.identityHint}
                  reference={match.candidate.benchReference}
                  sourceUrl={match.candidate.sourceUrl}
                  sourceLabel="Открыть специалиста"
                />
              </div>
          ...
          <Panel title="Результаты" subtitle="">
            <InlineEmptyState text={scope === "own" ? "Совпадений по нашему бенчу пока нет." : "Совпадений по бенчу партнёров пока нет."} />
          </Panel>
        )}
      </div>
    </section>
          ...
          {result ? (
            <div className="run-results">
              <div className="result-card result-card--summary">
                ...
                {result.matched_profile ? <p><strong>Профиль:</strong> {result.matched_profile}</p> : null}
              </div>
              {(result.sections?.length ? result.sections : [{ id: "default", title: "Результаты", empty_text: "Совпадений нет", items: result.items || [] }]).map((section) => (
                <div key={section.id} className="result-card">
                  <div className="result-card__title">{section.title}</div>
                  {section.items.length ? (
                    section.items.map((item) => (
                      <article key={`${section.id}-${item.id}`} className="run-result-item">
                        <div className="run-result-item__head">
                          <div>
                            <div className="run-result-item__title">{item.display_name || item.title}</div>
                            {item.display_name && item.role_label ? <div className="cell-subtitle">{item.role_label}</div> : null}
                            <div className="cell-subtitle">{item.subtitle}</div>
                          </div>
                          <Badge tone="purple">{item.score}%</Badge>
                        </div>
                        <div className="run-result-item__meta">
                          <span>{item.source_bucket_label ?? item.kind_label ?? item.kindLabel}</span>
                          <span>{item.meta}</span>
                        </div>
                        {item.identity_hint ? <div className="cell-subtitle">Ориентир в списке: {item.identity_hint}</div> : null}
                        {item.reference ? <div className="cell-subtitle">{item.reference}</div> : null}
                        {item.source_url ? (
                          <div className="run-result-item__meta">
                            <a href={item.source_url} target="_blank" rel="noreferrer">
                              {mode === "vacancy" ? "Открыть специалиста" : "Открыть вакансию"}
                            </a>
                          </div>
                        ) : null}
                    ...
                    <InlineEmptyState text={section.empty_text || "Совпадений нет"} />
                  )}
                </div>
              ))}
            </div>
...
function SettingsPage({ settings, theme, partnerBenchSources, onThemeChange, onSettingsChange, onSave, onReset }) {
  return (
    <section className="page">
      ...
      <div className="settings-stack">
        ...
        <Panel title="Актуальные бенчи партнёров" subtitle="Прямые ссылки на действующие партнёрские бенчи из общего реестра">
          {partnerBenchSources?.length ? (
            <div className="settings-stack">
              {partnerBenchSources.map((item) => (
                <div key={`${item.name}-${item.source_url}`} className="dialog-card__row">
                  <span>{item.name}</span>
                  <strong>
                    <a href={item.source_url} target="_blank" rel="noreferrer">
                      Открыть бенч
            ...
            <p className="hint">Список ещё не загружен.</p>
          )}
        </Panel>
      </div>
    </section>
    ...
    <div className="dialog-backdrop" role="presentation" onClick={onClose}>
      <div className="dialog-card" role="dialog" aria-modal="true" aria-label={title} onClick={(event) => event.stopPropagation()}>
        ...
        <div className="dialog-card__body">
          {rows.map((row) => {
            const normalizedRow = Array.isArray(row) ? { label: row[0], value: row[1] } : row;
            const label = normalizedRow?.label ?? "—";
            const value = normalizedRow?.value || "—";
            const href = normalizedRow?.href;
...
function CompareCard({ title, name, subtitle, stack, grade, rate, location, identityHint, reference, sourceUrl, sourceLabel }) {
  return (
    <div className="compare-card">
      ...
      {location && location !== "—" ? <div className="cell-subtitle">Локация: {location}</div> : null}
      {identityHint && identityHint !== "—" ? <div className="cell-subtitle">Ориентир в списке: {identityHint}</div> : null}
      {reference ? <div className="cell-subtitle">{reference}</div> : null}
      {sourceUrl ? (
        <div className="compare-card__meta">
