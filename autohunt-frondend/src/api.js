...
function normalizeVacancy(item) {
  ...
  return attachMetadata([
    firstNonEmptyString(item.company, item.client, item.customer, item.title, item.role, "—"),
...
function normalizeBench(item) {
  ...
  return attachMetadata([
    firstNonEmptyString(item.name, item.candidate, item.specialist, item.role, item.source_display, "—"),
    toArray(item.stack ?? item.skills ?? item.tech_stack),
    firstNonEmptyString(item.grade, item.level, "—"),
    firstNonEmptyString(rate, "—"),
    firstNonEmptyString(item.location, item.city, "—"),
    firstNonEmptyString(item.status, "Активно"),
